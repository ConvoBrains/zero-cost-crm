...
function workBlocks(dayStart, offsetDays) {
  if (offsetDays === 0) {
    ...
    const logoutAt = new Date(
      Math.max(dayStart.getTime() + 60 * 60_000, now.getTime() - 20 * 60_000)
    );
    const loginAt = new Date(
      Math.max(dayStart.getTime() + 30 * 60_000, logoutAt.getTime() - 7 * 60 * 60_000)
    );
...
async function logEvent(
  client,
  { userId, sessionId, type, entityType, entityId, summary, payload, at }
) {
  await client.query(
    `
...
try {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM user_sessions WHERE user_id IN (SELECT id FROM users WHERE email = ANY($1::text[]))`,
      [SDR_EMAILS]
    );
    await client.query(
    ...
    for (const user of users) {
      ...
      const intensity = user.email.startsWith('rahul')
        ? 1.0
        : user.email.startsWith('neha')
          ? 0.9
          : 0.7;
