...
async function getCompany(
  ...
  const boot = await api<{ companies: CompanyRow[] }>('/api/bootstrap', { session });
...
describe('champion contact → company auto-move (issue #29)', () => {
  it('advances the company stage per the mapping when the champion status changes', async () => {
    ...
    const contact = await createContact(session, co.id, { champion: true });
    ...
    const after = await getCompany(session, co.id);
  ...
  it('does not move the company when a NON-champion contact status changes', async () => {
    const session = await loginAsCookie(SEED.founder);
    const co = await createCompany(session);
    const contact = await createContact(session, co.id, { champion: false });
  ...
  it('leaves companies.notes untouched when a contact note is patched', async () => {
    ...
    const co = await createCompany(session, { notes: originalNotes });
  ...
  it('logs a stage-change activity marked source=champion_contact on auto-move', async () => {
    ...
    }>(`/api/activity/lead/company/${co.id}`, { session });
...
async function championStageMoves(
  ...
  const lead = await api<{
    events: Array<{ eventType: string; payload: Record<string, unknown> }>;
  }>(`/api/activity/lead/company/${companyId}`, { session });
  expect(lead.status).toBe(200);
...
describe('champion auto-move matrix (issue #29)', () => {
  it('Rejected champion moves the company to Not Interested', async () => {
    const session = await loginAsCookie(SEED.founder);
    const co = await createCompany(session); // Lead Added
    const contact = await createContact(session, co.id, { champion: true });
    ...
    expect((await getCompany(session, co.id))?.stage).toBe('Not Interested');

    const moves = await championStageMoves(session, co.id);
  ...
  for (const stage of ['Closed Won', 'Closed Lost'] as const) {
    it(`does not move a company already at ${stage} (closed-company guard)`, async () => {
      const session = await loginAsCookie(SEED.founder);
      const co = await createCompany(session, { stage });
      ...
      expect((await getCompany(session, co.id))?.stage).toBe(stage);
      expect(await championStageMoves(session, co.id)).toHaveLength(0);
  ...
  it('does not move when the company is already AHEAD of the mapped stage', async () => {
    const session = await loginAsCookie(SEED.founder);
    const co = await createCompany(session, { stage: 'Follow-up' });
    const contact = await createContact(session, co.id, { champion: true });
    ...
    expect((await getCompany(session, co.id))?.stage).toBe('Follow-up');
    expect(await championStageMoves(session, co.id)).toHaveLength(0);
    expectMovedStage(patched.data, null);
  ...
  it('does not move when the company is already AT the mapped stage (strictly forward only)', async () => {
    const session = await loginAsCookie(SEED.founder);
    const co = await createCompany(session, { stage: 'Discovery Call Done' });
    const contact = await createContact(session, co.id, { champion: true });
    ...
    expect((await getCompany(session, co.id))?.stage).toBe('Discovery Call Done');
    expect(await championStageMoves(session, co.id)).toHaveLength(0);
    expectMovedStage(patched.data, null);
  ...
  it('a same-status PATCH neither moves the company nor logs a second stage change', async () => {
    ...
    expect((await getCompany(session, co.id))?.stage).toBe('Discovery Call Done');
    expect(await championStageMoves(session, co.id)).toHaveLength(1);
